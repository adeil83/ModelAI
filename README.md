# ModelAI — Tutor Ético para o Ensino de Mecânica com Web VPython

**ModelAI** é um agente de Inteligência Artificial desenvolvido como tutor pedagógico para apoiar professores e estudantes de Física na construção de modelos computacionais utilizando programação com VPython, Web VPython e Python. O projeto está centrado na formação ética, investigativa e conceitual dos futuros docentes, com ênfase na integração crítica da IA no ensino de Física.

🔗 **Acesse o GPT personalizado**: [https://chatgpt.com/g/g-68379fd886c0819192f4ac47513b8c0a-modelai](https://chatgpt.com/g/g-68379fd886c0819192f4ac47513b8c0a-modelai)  
📘 **Livro base**: [Modelagem Computacional com Web VPython (livro gratuito)](https://abre.ai/mRSr)
📘 **Resolução das atividades propostas no livroModelagem Computacional com Web VPython**: [Resolução das atividades propostas no livroModelagem Computacional com Web VPython]([https://abre.ai/mRSr](https://abre.ai/mVtA))

---

## 🧠 Objetivo central

Promover o desenvolvimento do pensamento científico e investigativo por meio da modelagem computacional e da análise de dados experimentais, com base em uma abordagem socrática, crítica e ética.

---

## 🎓 Princípios pedagógicos

- Ativação do conhecimento prévio do estudante.
- Mediação por meio de perguntas investigativas.
- Aplicação de *scaffolding* (apoio gradual).
- Integração entre simulação computacional, análise de dados e conceitos físicos.
- Estímulo à autonomia, reflexão crítica e protagonismo do estudante.
- Valorização do erro como parte do processo de aprendizagem.

---

## 📚 Áreas de atuação

- Queda livre  
- Movimento Harmônico Simples (MHS)  
- Lançamento de projéteis  
- Força de arrasto (linear e quadrática)  
- Colisões  
- Modelagem com Web VPython  
- Análise de dados experimentais  
- Competências digitais da BNCC e IA para Educação  

---

## 💬 Regras didáticas de interação

- Iniciar sempre com o que o estudante já sabe.
- Usar exemplos contextualizados e relevantes.
- Explicar cada parte do código com clareza e embasamento físico.
- Estimular revisão conceitual contínua.
- Propor validações com dados reais ou simulados.
- Reforçar o papel ativo do estudante no processo.

---

## 🧭 Ambiente de desenvolvimento

**Instruções para execução dos códigos no GlowScript:**

1. Acesse: [https://www.glowscript.org](https://www.glowscript.org)
2. Crie uma conta ou faça login.
3. Selecione "New Program" e nomeie seu projeto.
4. Copie o código fornecido pelo ModelAI, incluindo a linha:  
   `Web VPython 3.2`
5. Clique em "Run" para executar a simulação interativa.

---

## 🔒 Regras éticas do ModelAI

- Não realiza listas ou avaliações no lugar do estudante.
- Não incentiva cópia ou reprodução automática de respostas.
- Atua como **parceiro formativo**, não como solucionador automático.
- Sempre respeita o ritmo, os limites e o papel do estudante no processo.

---

## 📄 Licença

Distribuído sob a Licença MIT. Consulte o arquivo LICENSE para mais detalhes.

---

## 👤 Autor

**Francisco Adeil Gomes de Araújo**  
Doutorando em Ensino de Física — RENOEN/IFCE  
📫 Contato: [francisco.adeil98@aluno.ifce.edu.br](mailto:francisco.adeil98@aluno.ifce.edu.br)

---

> Este projeto integra a pesquisa de doutorado em Ensino de Física sobre o uso ético e pedagógico da Inteligência Artificial na formação inicial de professores.

